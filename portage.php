...
if (isset($_GET[cash_on_id])){
	$_SESSION[cash_on_id] =  $_GET[cash_on_id];
}
$cash_on_id =   $_SESSION[cash_on_id];
...
if ($_POST['Updatemember']) {
	...
	if ($member_Reult) {
		...
		echo"<script>alert('อัพเดทเรียบร้อยแล้ว'); window.history.back();</script>";
	}
	else {
		echo"<script>alert('อีเมลที่ต้องการอัพเดทมีอยู่ในระบบแล้ว'); window.history.back(); </script>";
	}
}
...
if (isset($_POST[submit_order])) {
	...
	VALUES (NULL, '$_SESSION[member_online_id]', '$_SESSION[order_list_price]', '$_SESSION[total]', '$_SESSION[weight_price]', '$_SESSION[amount]', NOW(), NOW(), '$order_list_status', 'รอการจัดส่ง', '$_SESSION[order_list_weight]','$member[member_name]', '$member[member_email]', '$member[member_address]', '$member[member_zipcode]', '$member[member_phone]', '$order_list_p_name','$order_list_p_link', '2', '$order_list_cash_on_name', '1');";
									...
									<div class="form-group">
										...
										<div class=" col-md-10 text-left">
											<button type="button" class="btn btn-default " data-toggle="modal" data-target="#Updatemember">
												...
												<? if($_SESSION[Language]== "Thailand"){echo "อัพเดทข้อมูล"; } else{echo "Update"; } ?>
											</button> 
										</div>
									</div>
								</div>
