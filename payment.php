...
if ($_POST['submit_paymet']) {
	...
	if ($order_list_Row > 0 && $order_list[order_list_status] != 4 ) {
		...
		$payment_photo = "payment_photo".rand()."X".rand().$_FILES["payment_photo"]["name"];
		if(move_uploaded_file($_FILES["payment_photo"]["tmp_name"],"cloud/payment_photo/".$payment_photo)){
			...
			$payment_Add = "INSERT INTO `payment` (`payment_id`, `order_list_id`, `payment_price`, `payment_date`, `payment_time`,`payment_photo`,`payment_now`,`account_id`) VALUES (NULL, '$order_list_id', '$payment_price', NOW(), NOW(),'$payment_photo',NOW(), '$account_id')";
...
<head>
	...
	<title> <? if($_SESSION[Language]== "Thailand"){echo "แจ้งโอนเงิน"; } else{echo "payment"; } ?> <? echo $fixed[fixed_company]; ?> - <? echo $fixed[fixed_topic]; ?> | <? echo $fixed[fixed_website]; ?> </title>
				...
				<div class="panel panel-default no-boxsha radius20" style="overflow: hidden;">
					<div class="panel-body bg-white ">
						<div class="row">
							<div class="col-md-12">
								<p class="pagetopic">
									<? if($_SESSION[Language]== "Thailand"){echo "ชำระเงินโดยการโอนเงินไปที่"; } else{echo "account"; } ?>
								</p>
							</div>
							<?
							...
							while ($account 	= mysqli_fetch_array($account_QR)) {
								?>
								<div class="col-xs-12 size18 text1">
									<p>
										<span class="bold"><? if($_SESSION[Language]== "Thailand"){echo "ธนาคาร"; } else{echo "Bank"; } ?> : </span><?php echo $account[account_bank]; ?>
									</p>
									<p>
										<span class="bold"><? if($_SESSION[Language]== "Thailand"){echo "เลขบัญชี"; } else{echo "account number"; } ?> :  </span><?php echo $account[account_number]; ?>
									</p>
									<p>
										<span class="bold"><? if($_SESSION[Language]== "Thailand"){echo "ชื่อบัญชี"; } else{echo "account name"; } ?> :   </span><?php echo $account[account_name]; ?>
									</p>
								</div>
								<?
					...
					<div class="panel panel-default no-boxsha radius20" style="overflow: hidden;">
						<div class="panel-body bg-white  ">
							<div class="row">
								<div class="col-md-12 resize">
									<p class="pagetopic">
										<? if($_SESSION[Language]== "Thailand"){echo "แจ้งโอนเงิน"; } else{echo "proof of transfer"; } ?> 
									</p>
								</div>
							</div>
							<form action="" method="post" enctype="multipart/form-data">
								<div class="form-group">
									<label class="control-label">
									 ...
									 <span class="text-red">  <? if($_SESSION[Language]== "Thailand"){echo "( ดูได้ที่อีเมลของคุณ หรือาจอยู่ในจดหมายขยะ )"; } else{echo "( see your email or may be in spam)"; } ?>  </span>
									</label>
									<input   type="number" name="order_list_id" class="form-control" required  minlength="4"  maxlength="50">
								</div>
								<div class="form-group">
									<label class="control-label"> <? if($_SESSION[Language]== "Thailand"){echo "เลขบัญชีปลายทาง"; } else{echo "Bank"; } ?>  </label>
									<select class="form-control"  name="account_id" required>
										<option value=""> <? if($_SESSION[Language]== "Thailand"){echo "กรุณาเลือก เลขบัญชีปลายทาง"; } else{echo "Please select account "; } ?> </option>
										<?
								...
								<div class="form-group">
									<label class="control-label"><? if($_SESSION[Language]== "Thailand"){echo "แจ้งโอนเงิน"; } else{echo "proof of transfer"; } ?> </label>
									<input required  name="payment_photo" type="file" class="form-control">
								</div>
								<button type="submit" class="btn btn-main boxsha btn-block text1"><? if($_SESSION[Language]== "Thailand"){echo "แจ้งโอนเงิน"; } else{echo "submit"; } ?></button>
								<input type="hidden" name="submit_paymet"  value="x">
							</form>
						</div>
					</div>
				</div>
		...
		<div class="row hidden-sm hidden-xs margintop30" >
			<div class="col-md-12">
				<ul class="breadcrumb" style="margin-bottom: 0px;">
					...
					<li class="active"><a href="payment.php"> <? if($_SESSION[Language]== "Thailand"){echo "แจ้งโอนเงิน"; } else{echo "payment"; } ?> </a></li>
				</ul>
			</div>
		</div>
	</div>
